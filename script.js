...
function setlink(letter, input) {
  if (input.value.startsWith("http")) {
    return "https://" + letter + ".cryodream.dev/main/" + input.value
  } else if (input.value) {
    return "https://" + letter + ".cryodream.dev/main/https://www.google.com/search?q=" + input.value
  } else {
    alert('Please provide a link. It does not require https://.');
...
function direct() {
  ...
  var link;
  if (proxy === "wom") {
    link = setlink("w", input)
  } else if (proxy === "uv") {
    alert("Opening direct currently does not work with ultraviolet, sorry.")
    //link = setlink("u", input)
  } else if (proxy === "ram") {
    if (getCookie("ram-alert") !== "0") {
